 ...
 * First Last: Jessica Novillo Argudo
 * collaborators: Marieke Thomas, Yanique Sears
 */
...
public static int[] buildIncreasingArray( int size, int startValue, int step )
  ...
  */
  ...
  {
    int initialArray[] = Arrays.copyOf(data, data.length);
    for (int i=0; i<data.length; i++){
      data[i] = initialArray[data.length - 1 - i];
    }    
    printArray(data);
  }
  ...
  public static void main( String[] args )
  {
    ...
    System.out.println("First occurence index for number 10 is: " + index);
    ...
    System.out.println("There are " + oddNum + 
